### Hi there 👋
# 👋 Hi, I'm Salvin  

🎓 I am a **BCA graduate** from Kerala.  
💡 Passionate about **Data Analytics, Python, SQL, and Machine Learning**.  
🌱 Currently learning **pandas, NumPy, and data visualization**.  
⚡ Fun fact: I love hiking, gaming, and music.  

---

## 🚀 Skills & Tools  
- **Languages:** Python, SQL, Java, C  
- **Data Analysis:** Pandas, NumPy, Matplotlib  
- **Databases:** MySQL, SQLite  
- **Other Tools:** Git, Excel  

---

## 📌 Projects  
🔹 [Spotify Data Analysis](#) – Analyzing most-streamed songs using pandas  
🔹 [Sales Dashboard](#) – SQL + Excel dashboard for sales insights  
🔹 [Python Mini Projects](#) – Small utilities & problem-solving scripts  

---

## 📈 GitHub Stats  
![Salvin's GitHub Stats](https://github-readme-stats.vercel.app/api?username=salvvvi&show_icons=true&theme=radical)  

---

## 🌍 Let's Connect  
- 💼 [LinkedIn](www.linkedin.com/in/salvin-s-a81500314)  
- 📧 Email: salvinsalvin503@gmail.com  

✨ *"Data is the new oil, but insights are the new gold."*  



